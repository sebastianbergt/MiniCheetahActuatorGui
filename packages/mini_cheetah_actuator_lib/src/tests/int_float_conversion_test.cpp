...
SCENARIO("Actuator: Helper functions for bit banging") {
  GIVEN("values to convert") {
    ...
    WHEN("conversions are called") {
      ...
      const auto roundtrip_value =
          sut::toFloat(integer_value, X_MIN, X_MAX, BITS_USED);
      THEN("the roundtrip value is limited to boundaries or equals the input "
           "value when within boundaries") {
