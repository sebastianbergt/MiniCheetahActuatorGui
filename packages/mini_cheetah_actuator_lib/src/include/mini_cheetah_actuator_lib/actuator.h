...
namespace mini_cheetah_actuator_lib
{
    ...
    {
    public:
        Actuator(std::string can_interface_name, MotorId motor_id, std::unique_ptr<can_interface_lib::ICan> can = nullptr);
        ~Actuator();
        ...
        bool setPosition(AngleDeg position, VelocityDegPerSecond velocity, PositionFeedbackGain position_feedback_gain, VelocityFeedbackGain velocity_feedback_gain, FeedForwardCurrentAmpere feed_forward_current) override;
        bool setPosition(AngleRad position, VelocityRadPerSecond velocity, PositionFeedbackGain position_feedback_gain, VelocityFeedbackGain velocity_feedback_gain, FeedForwardCurrentAmpere feed_forward_current) override;
    ...
    private:
        ...
        can_interface_lib::CanFrame serialize(const AngleRad position, const VelocityRadPerSecond velocity, const PositionFeedbackGain position_feedback_gain, const VelocityFeedbackGain velocity_feedback_gain, const FeedForwardCurrentAmpere feed_forward_current);
        ...
        FeedForwardCurrentAmpere feed_forward_current_{
            0.0F};
    };
} // namespace mini_cheetah_actuator_lib
