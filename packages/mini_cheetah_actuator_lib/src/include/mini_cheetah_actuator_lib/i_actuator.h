...
namespace mini_cheetah_actuator_lib
{
    ...
    using VelocityRadPerSecond = strong_types::StrongType<float, struct VelocityRadPerSecondTag>;
    ...
    using FeedForwardCurrentAmpere = strong_types::StrongType<float, struct FeedForwardCurrentAmpereTag>;
    ...
    std::unique_ptr<IActuator> makeActuator(std::string can_interface, MotorId motor_id);
    ...
    public:
        ...
        virtual bool setPosition(AngleDeg position, VelocityDegPerSecond velocity, PositionFeedbackGain position_feedback_gain, VelocityFeedbackGain velocity_feedback_gain, FeedForwardCurrentAmpere feed_forward_current) = 0;
        virtual bool setPosition(AngleRad position, VelocityRadPerSecond velocity, PositionFeedbackGain position_feedback_gain, VelocityFeedbackGain velocity_feedback_gain, FeedForwardCurrentAmpere feed_forward_current) = 0;
