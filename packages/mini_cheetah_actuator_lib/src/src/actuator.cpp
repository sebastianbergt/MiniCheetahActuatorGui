...
Actuator::Actuator(std::string can_interface_name, MotorId motor_id,
                   can_interface_lib::ICan &can)
    : can_interface_name_{can_interface_name}, motor_id_{motor_id}, can_{can} {}

Actuator::~Actuator() { can_.disconnect(); }
...
bool Actuator::disable() { return send_magic_number(0xFD); }

bool Actuator::setCurrentPositionToZero() { return send_magic_number(0xFE); }

can_interface_lib::CanFrame
Actuator::serialize(const AngleRad position,
                    const VelocityRadPerSecond velocity,
  ...
  const std::uint32_t position_int{
      toInt(position.get(), POSITION_MIN, POSITION_MAX, POSITION_BITS)};
  const std::uint32_t velocity_int{
      toInt(velocity.get(), VELOCITY_RAD_PER_SECOND_MIN,
            VELOCITY_RAD_PER_SECOND_MAX, VELOCITY_RAD_PER_SECOND_BITS)};
  const std::uint32_t position_feedback_gain_int{
      toInt(position_feedback_gain.get(), POSITION_FEEDBACK_GAIN_MIN,
            POSITION_FEEDBACK_GAIN_MAX, POSITION_FEEDBACK_GAIN_BITS)};
  const std::uint32_t velocity_feedback_gain_int{
      toInt(velocity_feedback_gain.get(), VELOCITY_FEEDBACK_GAIN_MIN,
            VELOCITY_FEEDBACK_GAIN_MAX, VELOCITY_FEEDBACK_GAIN_BITS)};
  const std::uint32_t feed_forward_current_int{
      toInt(feed_forward_current.get(), FEED_FORWARD_CURRENT_MIN,
            FEED_FORWARD_CURRENT_MAX, FEED_FORWARD_CURRENT_BITS)};
...
constexpr float deg_to_rad(const float &in) { return in * (M_PI / 180.0F); }

bool Actuator::setPosition(
    const AngleDeg position, const VelocityDegPerSecond velocity,
    const PositionFeedbackGain position_feedback_gain,
    const VelocityFeedbackGain velocity_feedback_gain,
    const FeedForwardCurrentAmpere feed_forward_current) {
  ...
  const auto frame =
      serialize(position_in_rad, velocity_in_rad, position_feedback_gain,
                velocity_feedback_gain, feed_forward_current);
  return can_.send(frame);
}

bool Actuator::setPosition(
    const AngleRad position, const VelocityRadPerSecond velocity,
    const PositionFeedbackGain position_feedback_gain,
    const VelocityFeedbackGain velocity_feedback_gain,
    const FeedForwardCurrentAmpere feed_forward_current) {
  const auto frame = serialize(position, velocity, position_feedback_gain,
                               velocity_feedback_gain, feed_forward_current);
  return can_.send(frame);
}
...
bool Actuator::setPosition(const AngleDeg position) {
  const AngleRad position_in_rad{deg_to_rad(position.get())};
  const auto frame =
      serialize(position_in_rad, velocity_rad_, position_feedback_gain_,
                velocity_feedback_gain_, feed_forward_current_);
  return can_.send(frame);
}

bool Actuator::setPosition(const AngleDeg position,
                           const VelocityDegPerSecond velocity) {
  const AngleRad position_in_rad{deg_to_rad(position.get())};
  const VelocityRadPerSecond velocity_in_rad{deg_to_rad(velocity.get())};
  const auto frame =
      serialize(position_in_rad, velocity_in_rad, position_feedback_gain_,
                velocity_feedback_gain_, feed_forward_current_);
  return can_.send(frame);
}

bool Actuator::setPosition(const AngleRad position) {
  const auto frame = serialize(position, velocity_rad_, position_feedback_gain_,
                               velocity_feedback_gain_, feed_forward_current_);
  return can_.send(frame);
}
...
bool Actuator::setPosition(const AngleRad position,
                           const VelocityRadPerSecond velocity) {
  const auto frame = serialize(position, velocity, position_feedback_gain_,
                               velocity_feedback_gain_, feed_forward_current_);
  return can_.send(frame);
}
...
bool Actuator::getStatus(Status &status) {
  ...
  const auto velocity_int =
      (frame.data[3] << 4U) | ((0xF0 & frame.data[4]) >> 4U);
  const auto current_int = ((0x0F & frame.data[4]) << 8) | frame.data[5];

  status.motor_id = MotorId{motor_id};
  status.positon =
      AngleRad{toFloat(positon_int, POSITION_MIN, POSITION_MAX, POSITION_BITS)};
