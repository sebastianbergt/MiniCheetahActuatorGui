...
ssize_t write(int fd, const void *frame, size_t len) {
  return c_mock_linux_socket.write(fd, frame, len);
}

ssize_t read(int fd, void *frame, size_t len) {
  return c_mock_linux_socket.read(fd, frame, len);
}

int close(int fd) { return c_mock_linux_socket.close(fd); }
    ...
    WHEN("isConnected is called without calling connect before") {
      ...
      THEN("it is not connected") {
        ...
        AND_WHEN("connect is called") {
          REQUIRE_CALL(c_mock_linux_socket,
                       socket(eq(PF_CAN), eq(SOCK_RAW), eq(CAN_RAW)))
              .RETURN(MOCK_SOCKET);
          REQUIRE_CALL(c_mock_linux_ioctl,
                       ioctl(eq(MOCK_SOCKET), eq(SIOCGIFINDEX), _))
              .LR_SIDE_EFFECT(_3->ifr_ifindex = MOCK_CAN_INTERFACE_INDEX)
              .RETURN(RESULT_SUCCESS);
          REQUIRE_CALL(c_mock_linux_socket,
                       bind(eq(MOCK_SOCKET), _, sizeof(sockaddr_can)))
              .RETURN(RESULT_SUCCESS);
            ...
            AND_WHEN("isConnected is called") {
              ...
              THEN("it is connected") {
                ...
                AND_WHEN("send is called") {
                  REQUIRE_CALL(c_mock_linux_socket,
                               write(eq(MOCK_SOCKET), _, sizeof(can_frame)))
                      .RETURN(can_interface_lib::CAN_DATA_FRAME_SIZE);

                  const auto frame = can_interface_lib::makeCanFrame();
                  const auto send_success = can->send(frame);
                  THEN("send succeeded") {
                    REQUIRE(send_success);

                    AND_WHEN("filter is called") {
                      const auto success =
                          can->filter(can_interface_lib::CanId{0x100},
                                      can_interface_lib::CanMask{0x110});
                      THEN("filter succeeds") {
                        ...
                        AND_WHEN("receive is called") {
                          REQUIRE_CALL(
                              c_mock_linux_socket,
                              read(eq(MOCK_SOCKET), _, sizeof(can_frame)))
                              .RETURN(can_interface_lib::CAN_DATA_FRAME_SIZE);

                          auto frame = can_interface_lib::makeCanFrame();
                          const auto receive_success = can->receive(frame);
                          THEN("receive succeeds") {
                            REQUIRE(receive_success);

                            AND_WHEN("disconnect is called") {
                              REQUIRE_CALL(c_mock_linux_socket,
                                           close(eq(MOCK_SOCKET)))
                                  .RETURN(RESULT_SUCCESS);
                              ...
                              THEN("disconnect succeeded") {
                                ...
                                AND_WHEN(
                                    "isConnected is called after disconnect") {
                                  is_connected = can->isConnected();
                                  THEN("it is not connected") {
                                    REQUIRE_FALSE(is_connected);
                                  }
...
SCENARIO("Rainy day: Not connected first") {
  ...
  GIVEN("A CAN object") {
    ...
    WHEN("disconnect is called before being connected") {
      ...
      THEN("disconnect fails") { CHECK_FALSE(success); }
    }

    WHEN("send is called before being connected") {
      const auto frame = can_interface_lib::makeCanFrame();
      const auto success = can->send(frame);
      THEN("send fails") { CHECK_FALSE(success); }
    }

    WHEN("receive is called before being connected") {
      auto frame = can_interface_lib::makeCanFrame();
      const auto success = can->receive(frame);
      THEN("receive fails") { CHECK_FALSE(success); }
    }
    WHEN("filter is called before being connected") {
      const auto success = can->filter(can_interface_lib::CanId{0x100},
                                       can_interface_lib::CanMask{0x110});
      THEN("filter fails") { CHECK_FALSE(success); }
    }
  }
}
...
SCENARIO("Rainy day: connect failures") {
  GIVEN("A CAN object") {
    ...
    WHEN("connect is called and socket returns error") {
      ...
      const auto connect_success = can->connect("can0");
      THEN("connect failed") { CHECK_FALSE(connect_success); }
    }
...
SCENARIO("Rainy day: socket communication failures") {
  GIVEN("A CAN object") {
    ...
    WHEN("send is called and socket returns error") {
      ...
      THEN("send failed") { CHECK_FALSE(send_success); }
    }
    ...
    WHEN("receive is called and socket returns error") {
      ...
      THEN("receive failed") { CHECK_FALSE(receive_success); }
    }

    WHEN("disconnect is called and socket returns error") {
      REQUIRE_CALL(c_mock_linux_socket, close(eq(MOCK_SOCKET)))
          .RETURN(RESULT_FAILURE);

      const auto disconnect_success = can->disconnect();

      THEN("disconnect failed") { CHECK_FALSE(disconnect_success); }
    }
  }
}
